...
export default function WorkingWithArrays() {
    ...
    return (
        <div id="wd-working-with-arrays">
            ...
            <input defaultValue={todo.description} className="form-control w-75" onChange={(e) => setTodo({ ...todo, description: e.target.value })} />
            <br />
            <a href={`${API}/${todo.id}/completed/${todo.completed}`} className="btn btn-primary float-end">
                Update Completed Status</a>
            <input type="checkbox" id="completed" defaultChecked={todo.completed} onChange={(e) => setTodo({ ...todo, completed: e.target.checked })} />
